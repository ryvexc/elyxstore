...
export async function getServerSideProps({ query, req }: any) {
  const kodebrg: string = query.kodebrg || "";
  let dataBarang;

  const cookieData = cookie.parse(req.headers.cookie);
  await fetch(`http://localhost:3000/api/dataBarang?barang=&toko=${JSON.parse(cookieData.userdata).nama.replace(" ", "+")}&kodebrg=${kodebrg}`).then(res => res.json()).then(data => {
    dataBarang = data;
  });
...
export default function Index({ cookieData, kodebrg, dataBarang }: any): JSX.Element {
  ...
  React.useEffect(() => {
    setGambarSrc(dataBarang[0].gambar);
  ...
  return <form action="/api/editBarang" method="POST">
    <input type="hidden" name="kodebrg" value={kodebrg} />
    <Background></Background>
    <div className="w-full justify-center">
      <h1 className="text-xl text-center my-10 font-medium">Edit Item - <span className="text-2xl font-bold">{dataBarang[0].nama}</span></h1>
