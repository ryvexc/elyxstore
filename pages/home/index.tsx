...
import { faCartShopping, faMagnifyingGlass, faHouse, faUser, faClockRotateLeft } from "@fortawesome/free-solid-svg-icons";
...
export default function Index({ barang }: { barang: string }) {
  ...
  React.useEffect(() => {
    fetch(`/api/dataBarang?barang=${barang}`)
      ...
      .then((data: any): void => {
        setContent(<Home stateBarang={data} searchMode={!!barang} />);
        setDataBarang(data);
      })
